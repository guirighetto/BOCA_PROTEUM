...
function DBNewProblem($contestnumber, $param, $c=null) {
	...
	$ac1=array('colorname','fake','color','updatetime','fullname',
			   'basename','inputfilename','inputfilepath','problemtype');
	$problemtype='';
	$colorname='';
	$color='';
	$fake='f';
...
// "and probleminputfilename='$inputfilename'";
	...
	if($s=="created" || $updatetime > $t) {
		...
		if ($problemtype != "") {
			$tmpProblemType = intval($problemtype);
			$deservesupdatetime=true;
			DBExec ($c, "update problemtable set problemtype='$tmpProblemType' where contestnumber=$contestnumber ".
					"and problemnumber=$number", "DBNewProblem(update problemtype)");
		}
		if ($colorname != "")
